...
class _FeedMeState extends State<FeedMe> {
  ...
  getFeedbackFromSheet() async {
   ...
   // print(jsonfeedback);
    setState(() {
      catagorylist =
          jsonfeedback[0].map((item) => item['name'].toString()).toList();
      specialnames =
          jsonfeedback[1].map((item) => item['name'].toString()).toList();
      specialid = jsonfeedback[1].map((item) => item['id'].toString()).toList();
      specialingredients = jsonfeedback[1]
          .map((item) => item['ingredients'].toString())
          .toList();
      specialvideos =
          jsonfeedback[1].map((item) => item['video'].toString()).toList();
      specialprices =
          jsonfeedback[1].map((item) => item['price'].toString()).toList();
    });
  }
                    ...
                    child: DropdownButton<String>(
                      ...
                      onChanged: (value) {
                        setState(() {
                          selectedLang = value
                              .toString(); 
                           int selectedIndex = lang.indexOf(selectedLang);
                       if(selectedIndex==0)
                       {
                      context.setLocale(
                                      const Locale("am", "ET"),
